...
class OcrService {
  constructor({ cachePath }) {
    ...
    // 当前识别调用的进度回调。并发识别时后调用的 recognize 会覆盖它；
    // worker 的 logger 始终读取最新值，且只有持有者能清理，避免在途帧
    // 的 finally 清掉新调用的回调。
  ...
  async ensureWorker(languages) {
    ...
    this.worker = await createWorker(languages, 1, {
      ...
      logger: (message) => {
        const handler = this.progressListener;
        if (!handler) return;
  ...
  async recognize(dataUrl, languages, onProgress) {
    ...
    try {
      ...
      if (this.progressListener === onProgress) this.progressListener = null;
    }
  }

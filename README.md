...
- **输入翻译面板新增「朗读文本」**：按钮位于「复制译文」左侧，点击后由 Windows 系统语音（SAPI）从默认输出设备朗读译文；再点一次、按 Esc、收起面板或开始新一次翻译都会立即停止。
- **按目标语言自动挑选语音**：英语用 en-US 语音、中文用 zh-CN 语音；目标语言在本机没有对应语音包时（如日语、俄语）回退到英语语音，并在状态栏如实说明实际使用的语音，不假装能读好。
- **零依赖且点开即读**：朗读由一个常驻的 PowerShell SAPI 宿主进程完成，面板打开时预热，实测「命令到出声」约 0～150 毫秒；停止采用终止宿主进程的方式，收起面板时进程一并回收，不会残留后台进程。
- **朗读文本全程 Base64 传输**：译文里的引号、竖线、换行、`$()` 等字符都无法破坏朗读协议或注入脚本。
- 新增 `lib/tts-service.js` 与 24 项单元测试（语言映射、语音回退、注入安全、宿主协议与生命周期），`npm test` 49/49 通过。

### 2026-09 v0.1.3

- **输入翻译面板新增"朗读文本"**：点击后通过 Windows 系统 SAPI 语音从默认输出设备朗读译文（自动按目标语言选择语音包），朗读中再点一次停止，关闭面板自动停止。
- **修复长译文溢出**：面板改为弹性布局，译文区域占据剩余空间并内部滚动——内容再多，"复制/朗读"按钮始终可见，滚轮可直接浏览。

### 2026-09 v0.1.4：覆盖层重写（对齐 QQ 截图翻译效果）

- **无缝覆盖**：从截图采样每个文本块周围的背景色与方差，背景平坦时把块填充为原始背景色 + 对比文字色（无边框/阴影），译文与屏幕融为一体；背景复杂时自动回退深色块。
- **行合并成段**：OCR 行按垂直间距与水平对齐合并为段落块后整段翻译——句子完整、请求数大减、覆盖块不再互相压盖。
- **噪声治理三件套**：符号汤/低置信度行过滤、纯拉丁碎片（<3 字母）过滤、URL/路径形态过滤；全屏翻译只保留前台窗口范围内的文本块（截屏时记录前台窗口矩形）。
- **防重叠重排**：块按从上到下处理，重叠时向下顺移且保证不超出屏幕底部；字号自适应收缩。
- **行内大空隙拆分**：同一 OCR 行内横向空隙超过 ~2.5 倍字高时拆分，避免并排窗口文本互相污染。
- 全屏截屏时隐藏自身结果窗口，杜绝"套娃"识别。
